...
Core training script for the multiplication task-specific NPI. Instantiates a model, then trains using
the precomputed data.
"""
from model.npi import NPI
from tasks.multiplication.multiplication import MultiplicationCore
from tasks.multiplication.env.config import CONFIG, get_args, ScratchPad
import pickle
...
DATA_PATH = "tasks/multiplication/data/train.pik"
LOG_PATH = "tasks/multiplication/log/"


def train_multiplication(epochs, verbose=0):
    """
    Instantiates an Multiplication Core, NPI, then loads and fits model to data.
    ...
    print ('initializing multiplication core ...')
    ...
    for ep in range(1, epochs + 1):
        for i in range(len(data)):
            ...
            for j in range(len(x)):
                ...
                if prog_in_id in [WRITEMUL, WRITEADD, PTRMUL, PTRADD]:
                ...
                # Fit!
                if prog_out_id in [WRITEMUL, WRITEADD, PTRMUL, PTRADD]:
        ...
        # Save Model
        saver.save(sess, 'tasks/multiplication/log/model.ckpt')